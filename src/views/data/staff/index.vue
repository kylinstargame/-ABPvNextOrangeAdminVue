        ...
        <el-table v-loading="loading" :data="staffList" @selection-change="handleSelectionChange">
	        ...
	        <el-table-column label="简介视频" align="center" prop="video">
                <template slot-scope="scope">
                    <video v-if="scope.row.video!= ''&&scope.row.video!= null" :src="scope.row.video"
                         style="width: 60px; height: 60px; padding-right: 1px" />
                    <el-button g v-else size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                               v-hasPermi="['system:role:edit']" >上传风采照片
                    </el-button>
                </template>
	        </el-table-column>
        ...
        <el-dialog :title="title" :visible.sync="open" width="1200px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                ...
                <el-form-item label="个人签名" prop="signature">
                    <el-upload :limit="1" :action="uploadUrl" :multiple="false"
                               list-type="picture-card"
                               :file-list="this.form.signatureList"
                               :on-remove="handleSigbatureRemove"
                               :before-upload="beforeImageUpload"
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getStaff(id).then(response => {
        ...
        this.form.signatureList.push({
          response: this.form.signature,
          url: this.form.signature,
          uid: this.form.signature.split('=').pop()
        })
        this.form.videoList = []
        this.form.videoList.push({
          response: this.form.video,
          url: this.form.video,
          uid: this.form.video.split('=').pop()
        })
        this.open = true
        this.title = '修改员工信息'
      })
    ...
    handlePhotosRemove(file, fileList) {
      ...
      this.form.photos.pop(file.response)
    },
    ...
    handleSigbatureRemove(file, fileList) {
